...
from src.preprocessing import add_weather_interactions, encode_station_features
from src.train_model import PM25LSTM
...
def engineer_db_history(measurements: pd.DataFrame) -> pd.DataFrame:
    ...
    return add_weather_interactions(pd.concat(parts, ignore_index=True))
...
def predict_latest(
    ...
    if not station_categories:
        station_categories = [
            feature.removeprefix("station__")
            for feature in bundle.feature_names
            if feature.startswith("station__")
        ]
    if station_categories:
        frame, _ = encode_station_features(
            frame,
            station_categories=station_categories,
        )
    frame = add_weather_interactions(frame)
