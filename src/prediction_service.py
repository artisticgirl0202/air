...
def get_artifact_bundle() -> ArtifactBundle:
    return load_artifacts("pm25_t24_ensemble")


def _risk_grade(pm25: float) -> str:
    if pm25 <= 15:
        return "좋음"
    if pm25 <= 35:
        return "보통"
...
def get_station_dashboard(
    ...
    *,
    ...
    if start_date is not None and end_date is not None:
        ...
        latest = latest_measurement_time(
            station_name,
        ...
        display_start = None
        end_exclusive = None
        latest = latest_measurement_time(station_name)
    if latest is None:
    ...
    if display_start is not None and latest_timestamp < display_start:
        raise InferenceError("No data found for the selected station and date range")
    history_hours = max(days * 24, 72)
    history_start = (
        min(display_start, latest_timestamp - pd.Timedelta(hours=72))
        if display_start is not None
        else latest_timestamp - pd.Timedelta(hours=history_hours)
    )
    history_end = min(
        end_exclusive,
        latest_timestamp + pd.Timedelta(hours=1),
    ) if end_exclusive is not None else latest_timestamp + pd.Timedelta(hours=1)
    ...
    if history.empty:
        raise InferenceError("No data found for the selected station and date range")
    engineered = engineer_db_history(history)
    bundle = get_artifact_bundle()
    ...
    return {
        ...
        "history": _history_records(history, display_start),
        "prediction": prediction_payload,
        "instance_shap_positive": positive_causes,
        "global_shap": global_shap,
        "recommendation": recommendation,
        "performance": get_model_performance(),
        "model_scope_warning": _model_scope_warning(station_name, bundle),
    }
