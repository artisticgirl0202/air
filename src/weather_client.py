...
def normalize_service_key(service_key: str) -> str:
    """Decode a portal-encoded key once before requests encodes query params.

    Public Data Portal commonly shows both encoded and decoded keys. Passing an
    already encoded key through ``params`` without decoding would turn ``%2B``
    into ``%252B``. ``unquote`` is intentionally used instead of
    ``unquote_plus`` so a literal plus sign is preserved.
    """
...
class KMAWeatherClient:
    ...
    def __init__(
        ...
        *,
        ...
        raw_key = service_key or os.getenv("KMA_SERVICE_KEY", "")
        if not isinstance(raw_key, str):
            raise TypeError("service_key must be a string")
        normalized_candidate = raw_key.strip().lower()
        if not normalized_candidate or normalized_candidate.startswith(
            _KEY_PLACEHOLDER_PREFIXES
        ):
            raise ValueError("KMA_SERVICE_KEY가 설정되지 않았습니다.")
        self._service_key = normalize_service_key(raw_key)
        ...
        self._verify_service_key_encoding()
    ...
    def _verify_service_key_encoding(self) -> None:
        prepared = requests.Request(
            "GET",
            KMA_ASOS_URL,
            params={"serviceKey": self._service_key},
        ).prepare()
        encoded_values = parse_qs(
            urlsplit(prepared.url or "").query,
            keep_blank_values=True,
        ).get("serviceKey", [])
        if encoded_values != [self._service_key]:
            raise KMAAPIError("KMA service key could not be encoded safely")
